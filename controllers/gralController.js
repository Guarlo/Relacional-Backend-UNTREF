...
//app.post('/actores/bulk', async (req, res) => { 
const bulkActors = async (req, res) => {
  try {
    const actors = req.body

    // Validar que se haya pasado un array y que no esté vacío
    if (!Array.isArray(actors) || actors.length === 0) {
      return res.status(400).json({ error: 'Debe proporcionar un array de actores' })
    }

    // Validar que cada actor tenga la propiedad "nombre" y no esté vacía
    const isValid = actors.every(actor => actor.nombre && typeof actor.nombre === 'string')
    if (!isValid) {
      return res.status(400).json({ error: 'Cada actor debe tener una propiedad "nombre" válida' })
    }

    // Crear los actores en la base de datos
    const createdActors = await Actor.bulkCreate(actors, { validate: true })
...
module.exports = { migrarTrailerflixJSON, getAllActors, bulkActors }